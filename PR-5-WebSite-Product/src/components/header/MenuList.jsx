...
const MainMenu = () => {
  ...
  const categories = [
    { name: "Dairy & Bakery", subcategories: { Dairy: ["Milk", "Ice Cream", "Cheese", "Frozen Custard", "Frozen Yogurt"], Bakery: ["Cake And Pastry", "Rusk Toast", "Bread & Buns", "Chocolate Brownie", "Cream Roll"] } },
    { name: "Fruits & Vegetable", subcategories: { Fruits: ["Apple", "Banana", "Mango", "Grapes"], Vegetables: ["Carrot", "Tomato", "Spinach", "Potato"] } },
    { name: "Snack & Spice", subcategories: { Snacks: ["Chips", "Cookies", "Nuts"], Spices: ["Turmeric", "Pepper", "Cumin"] } },
    { name: "Juice & Drinks", subcategories: { Drinks: ["Orange Juice", "Apple Juice", "Lemonade"] } },
      ...
      <Row className="w-100 align-items-center">
        ...
        <Col md="auto">
          <Dropdown className="d-inline-block">
            <Dropdown.Toggle variant="light" className="border px-3 py-2 rounded d-flex align-items-center">
              <RiMenu2Line size={20} className="me-2" />
                 
            </Dropdown.Toggle>

            <Dropdown.Menu className="p-3" style={{ width: "500px" }}>
              <Row>
                  ...
                  <ListGroup>
                    {categories.map((category) => (
                      <ListGroup.Item
                        key={category.name}
                        action
                        className={`border-0 ${selectedCategory === category.name ? "text-success fw-bold border border-success" : ""}`}
                    ...
                    .map((category) => (
                      <div key={category.name}>
                        {Object.keys(category.subcategories).map((subcategory) => (
                          <div key={subcategory}>
                            <h6 className="text-success fw-bold">{subcategory}</h6>
                            <ul className="list-unstyled">
                              {category.subcategories[subcategory].map((item) => (
